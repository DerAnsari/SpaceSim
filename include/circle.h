...
public:
  Circle(float radius = 1.0f, int segments = 32) {
    ...
    glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, 3 * sizeof(float), static_cast<void *>(nullptr));
    glVertexAttribDivisor(1, 1);

    glEnableVertexAttribArray(2);
    glVertexAttribPointer(2, 1, GL_FLOAT, GL_FALSE, 3 * sizeof(float), reinterpret_cast<void *>(2 * sizeof(float)));
  ...
  // Deprecated: old single-draw method it would work but result  in 4fps at 5000 bodies
  void draw(unsigned int shaderProgram, float x, float y, float scale) const {
