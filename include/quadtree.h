 ...
 * Requirement: Templates (Advanced C++ Feature)
...
public:
   ...
   */
  glm::vec2 calculateForce(T *target, float G, float softening, float theta) {
...
private:
  void subdivide() {
    ...
    children[0] = std::make_unique<QuadNode<T> >(xMin, xMid, yMid, yMax); // NW
    children[1] = std::make_unique<QuadNode<T> >(xMid, xMax, yMid, yMax); // NE
    children[2] = std::make_unique<QuadNode<T> >(xMin, xMid, yMin, yMid); // SW
    children[3] = std::make_unique<QuadNode<T> >(xMid, xMax, yMin, yMid); // SE
  }

  void insertIntoChild(T *b) {
    float xMid = (xMin + xMax) / 2.0f;
    float yMid = (yMin + yMax) / 2.0f;
